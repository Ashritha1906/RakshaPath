...
export default function VoiceCommand() {
  ...
  const handleVoiceCommand = async (transcript: string) => {
    ...
    try {
      const response = await axios.post('http://localhost:8000/api/voice/intent', { transcript });
      ...
      switch(intent) {
        case 'SOS':
          ...
          axios.post('http://localhost:8000/api/emergency/sos?user_id=1&lat=17.3850&lng=78.4867').catch(() => {});
          break;
        case 'GUARDIAN':
