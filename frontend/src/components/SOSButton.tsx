...
export default function SOSButton() {
  ...
  const handleSOS = async () => {
    ...
    try {
      ...
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/emergency/sos?user_id=1&lat=17.3850&lng=78.4867`);
      
      // 2. Fetch Nearest Safe Zones
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/safezones/discovery?lat=17.3850&lon=78.4867`);
