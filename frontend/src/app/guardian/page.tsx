...
export default function Guardian() {
  ...
  const fetchContacts = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/guardian/contacts?user_id=1`);
  ...
  const handleShare = async () => {
    setSharing(true);
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/guardian/share?user_id=1`);
  ...
  const addContact = async (e: React.FormEvent) => {
    ...
    if (newContact.name && newContact.phone) {
      try {
        await axios.post(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/guardian/contacts`, newContact);
